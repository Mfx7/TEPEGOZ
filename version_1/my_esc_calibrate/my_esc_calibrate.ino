...
void setup(){
  ...
  while(eeprom_data[33] != 'J' || eeprom_data[34] != 'M' || eeprom_data[35] != 'B'){
    ...
    digitalWrite(ledPin, !digitalRead(ledPin));                                         //Change the led status to indicate error.
  }
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        if(cal_int % 125 == 0){
          digitalWrite(30, !digitalRead(30));   //Change the led status to indicate calibration.
          Serial.print(".");
        }
...
void esc_pulse_output(){
  ...
  while(PORTH >= 8){                                            //Execute the loop until digital port 4 to 7 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTH &= B11110111;     //When the delay time is expired, digital port 4 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTH &= B11101111;     //When the delay time is expired, digital port 5 is set low.
    if(timer_channel_3 <= esc_loop_timer)PORTH &= B11011111;     //When the delay time is expired, digital port 6 is set low.
    if(timer_channel_4 <= esc_loop_timer)PORTH &= B10111111;     //When the delay time is expired, digital port 7 is set low.
  }
}
